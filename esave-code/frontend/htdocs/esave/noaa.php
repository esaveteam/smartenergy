...
class noaa_weather {
	...
	private function get_layout() {
		...
		$time_layouts = $this->xml->data[0]->{'time-layout'};
		$temps = $this->xml->data->parameters->temperature;

		for($i = 0; $i < count($time_layouts); $i++) {
			/* Find time_layout with more than 8 children, this is main layout */
#			if( count( $time_layouts[$i]->{'start-valid-time'} ) > 8 ) {
			if ($time_layouts[$i]->{'layout-key'}=='k-p24h-n7-1') {
				$this->main_label = $i;
			}
